...
bool ComponentLabelingSplit(	IplImage* inputImage,IplImage* maskImage,unsigned char backgroundColor,Blob_vector &blobs, int origin,int height )
{
	int i,j;
	// row major vector with visited points 
	bool *visitedPoints, *pVisitedPoints, internalContour, externalContour;
	...
	inputImage = &(IplImage)Mat(inputImage)(Rect(0,origin,inputImage->width,height));
	...
	labelledImage = (t_labelType*) malloc( inputImage->width * inputImage->height * sizeof(t_labelType) );
	visitedPoints = (bool*) malloc( inputImage->width * inputImage->height * sizeof(bool) );
	/*labelledImage = (t_labelType*) malloc( inputImage->width * height * sizeof(t_labelType) );
	visitedPoints = (bool*) malloc( inputImage->width * height * sizeof(bool) );*/
